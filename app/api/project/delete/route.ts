...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(getApiUrl("/api/project/delete"), {
      method: "POST",
      headers: {
    ...
    if (contentType && contentType.includes("application/json")) {
      ...
      if (!response.ok || data.status === "error") {
        throw new Error(data.message || `Failed to delete project: ${response.status}`)
      }
    ...
    console.error("Error in project/delete API route:", error)
